...
import { Check, ArrowRight, ArrowLeft, Building2, FileText, Loader2 } from "lucide-react";
import { authApi, isApiError, partnersApi } from "@/lib/api";
...
export default function DealerRegisterPage() {
  ...
  useEffect(() => {
    if (status === "authenticated" && user?.role === "PARTNER") {
      router.replace("/dealer");
    }
  }, [status, user?.role, router]);

  // Once authenticated, the auth/OTP steps don't apply — render step 3 directly.
  const effectiveStep: Step =
    status === "authenticated" && step !== 3 ? 3 : step;
  const displayName = name || (status === "authenticated" ? user?.name ?? "" : "");
  ...
  const handleStep1Continue = async () => {
    ...
    if (authMethod === "email") {
      ...
      setBusy(true);
      try {
        const resp = await authApi.registerEmail({ name: name.trim(), email, password });
        setSession(resp);
        setStep(3);
      } catch (err) {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          {effectiveStep === 2 && (
            <div className="space-y-5">
              ...
              <p className="text-center text-xs text-gray-500">
                ...
                <button
                  onClick={async () => {
                    ...
                    try { await authApi.requestOtp({ phone: e164Phone }); } catch (err) { handleApiError(err); } finally { setBusy(false); }
                  }}
              ...
              <div className="flex gap-3">
                ...
                >
                  {busy ? <Loader2 size={16} className="animate-spin" /> : <>Verify OTP <ArrowRight size={16} /></>}
                </button>
              </div>
            </div>
          )}
          ...
          {effectiveStep === 3 && (
            <div className="space-y-5">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setStep(authMethod === "phone" ? 2 : 1)}
